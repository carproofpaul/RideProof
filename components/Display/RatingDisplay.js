...
const RatingDisplay = ({vhrReport, recalls}) => {
     ...
     */
    if(vhrReport.TitleOrRegistrationEvents !== null){
        isRegistered = false
        isCommercial = false
        now = moment()
        
        for(i = 0; i < vhrReport.TitleOrRegistrationEvents.length; i++){
            then = moment(vhrReport.TitleOrRegistrationEvents[i].Date.split('T')[0])
            if( now.diff(then, 'years', true) <= 2 ){
                isRegistered = true
                isCommercial = vhrReport.TitleOrRegistrationEvents[i].UseType == 'Commercial_Use'
            }
        }
     ...
     */
    if(vhrReport.AccidentEvents !== null){
        numberOfAccidents = 0
        for(i = 0; i < vhrReport.AccidentEvents.length; i++){
            now = moment()
            then = moment(vhrReport.AccidentEvents[i].Date.split('T')[0])
     ...
     * null == no recalls
     */
    if(vhrReport.RecallEvents !== null){
        numberOfRecalls = 0
        for(i = 0; i < vhrReport.RecallEvents.length; i++){
            if( vhrReport.RecallEvents[i].Status === 'Outstanding' || vhrReport.RecallEvents[i].Status === 'Unknown'){
                numberOfRecalls++
            }
        }
     ...
     */
    if(vhrReport.ServiceEvents !== null){
        numberOfService = 0
        now = moment()
        for(i = 0; i < vhrReport.ServiceEvents.length; i++){
            then = moment(vhrReport.ServiceEvents[i].Date.split('T')[0])
            if( now.diff(then, 'years', true) <= 1 ){
                numberOfService++
            }
        }
    ...
    return (
        <View style={{alignItems: 'center'}}>
            ...
            <Text style={{fontSize: 25, fontWeight: 'bold', paddingBottom:10}}>{recalls.ModelYear + " " + recalls.Make + " " + recalls.Model}</Text>              
        </View>
    )
}
